      ...
      if ($row [0] == $_GET ["id"])
      {
  	  ...
  	  echo "
  	    <img src ='Albums/".$file."' style='width: 600' usemap='#map1' alt=''/><br/>  	      
  	      ...
  	      onBD ($result6, "PHOTO_PEOPLE");
              ...
              if ($row6 [1] == $_GET ["id"])
            {
              echo "<area href='data.php?id=".$row6 [2]."' shape='rect' coords='".$row6 [5].", ".$row6 [3].", ".$row6 [6].", ".$row6 [4]."'  onMouseOver='drawBorder (".$row6 [3].", ".$row6 [4].", ".$row6 [5].", ".$row6 [6].")' onMouseOut='returnPhoto (".$_GET ["id"].")' title='";
              onBD ($result7, "PEOPLE");
                ...
                while ($row4 = mysqli_fetch_array($result))
                  if (($row [2] == $row4 [0]) && ($row4 [2] == $row1 [3]))
                  {
                    ...
                    echo "<a href='JavaScript:;' onclick='$(\"#block-1 .imgWrap\").photoLabel(\"start\")'
                      class='turnOn' onMouseDown='addPhoto (".$_GET ["id"].", 0, 0, true)'>
        ...
        while ($row1 = mysqli_fetch_array ($result1, MYSQLI_NUM))
         ...
         while ($row1 = mysqli_fetch_array ($result1, MYSQLI_NUM))
           if ($row1 [2] == $row [2])
           {
             if (($row [0] - $row1 [0] > 0) && ($row1 [0] > $pred))
               $pred = $row1 [0];
             if (($row1 [0] - $row [0] > 0) && ($row1 [0] < $next))
               $next = $row1 [0];
           }
         if ($pred != -1)
           echo "<a href='detail_photo.php?id=".$pred."'><img src='lb-previous-active.png' style='border: 0'>
             </a>";
         echo "<a href='photo_list147.php?id=".$row [2]."'>Перейти к альбому</a>";
         if ($next != $i)
           echo "&nbsp;<a href='detail_photo.php?id=".$next."'><img src='lb-next-active.png'
             style='border: 0'></a> ";
         echo "</p>
           </td>
           </tr>";
